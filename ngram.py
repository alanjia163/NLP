...
def tokenize_raw(text):  # 先以标点符号为单位切分，再使用hanlp的seg_sentences分词
    split_sen = (i.strip() for i in re.split('。|,|，|：|:|？|！|\t|\n', _replace_c(text)) if
                 len(i.strip()) > 5)  # 这里用()而不用[] 是因为（）是生成器，有利于减小内存，如果用[]生成list的话可能会内存不足
...
if __name__ == '__main__':
    copus = [tokenize_raw(line.strip()) for line in open('text.txt', 'r', encoding='utf8') if
             len(line.strip()) > 0 and "RESUMEDOCSSTARTFLAG" not in line]
    ...
    freq1 = vectorizer1.fit_transform(doc)  # 计算词频
    word_freq = [freq1.getcol(i).sum() for i in range(freq1.shape[1])]

    transformer = TfidfTransformer()  # 这个类会统计每个词语 tf-idf
